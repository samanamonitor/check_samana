    ...
    for service in data['Services']:
        ...
        if re.search(incl, displayname) is not None or re.search(incl, name) is not None:
            if int(service['Status']) == 4:
                ...
                s += 1
                stopped_services += " * %s(%s)" % (service['DisplayName'], name)
...
def uptime(data, crit, warn):
    ...
    try:
        crit = float(crit)
        if val > crit:
            return (2, "CRITICAL - Uptime of server is %.0f Hours | %.0f;;;" % (val, val))
    except:
        pass
    ...
    try:
        warn = float(warn)
        if val > warn:
            return (1, "WARNING - Uptime of server is %.0f Hours | %.0f;;;" % (val, val))
    except:
        pass

    return (0, "OK - Uptime of server is %.0f Hours | %.0f;;;" % (val, val))
